...
void ARPGCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Save", IE_Pressed, this, &ARPGCharacter::SaveGame);
	PlayerInputComponent->BindAction("Load", IE_Pressed, this, &ARPGCharacter::LoadGame);
	...
	//Crouch
	PlayerInputComponent->BindAction("Crouch", IE_Pressed, this, &ARPGCharacter::DoCrouch);
	PlayerInputComponent->BindAction("Crouch", IE_Released, this, &ARPGCharacter::StopCrouch);
}


void ARPGCharacter::Destroyed() {

	const FVector Location = GetActorLocation();
	const FRotator Rotation = GetActorRotation();
	AActor* effect = GetWorld()->SpawnActor<AActor>(ActorSmoke, Location, Rotation);

	if (effect) {
		effect->SetLifeSpan(explosionLifeTime);
	}

	ARPGGameMode* gameMode = (ARPGGameMode*)GetWorld()->GetAuthGameMode();

	if (gameMode != NULL)
	{
		//if (GEngine)
			//GEngine->AddOnScreenDebugMessage(-1, 1.5, FColor::Green, TEXT("On Destroyed"));

		gameMode->OnPlayerDestroyed(GetController(), spawnPosition, spawnRotation);
	}
	Super::Destroyed();
}
